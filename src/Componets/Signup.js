...
import { UnPostRequest } from "../Utils/ServerHelper";
// first user from fronted password aliiscoder
// 2nd user from fronted password Iamfsali
const Signup = () => {
  ...
  const signupSubmit = async () => {
    if (email !== conformEmail) {
      alert("conformEmail is not correct");
    }
    const data = { email, password, username, firstName, lastName };
    const response = await UnPostRequest("/api/auth/register", data);
  ...
  return (
    <>
      <div className="w-ful h-full bg-gray-950 text-white flex flex-col items-center overflow-hidden">
        <div className="p-4 w-full h-65 items-center flex bg-gray-20">
        ...
        <div className="w-1/3 py-10 flex items-center  flex-col">
          ...
          <div className="w-full flex items-center justify-center my-8">
            <button
              className="bg-green-400 text-gray-950 font-semibold p-3 px-10 rounded-full"
              onClick={(e) => {
                e.preventDefault(e);
                signupSubmit();
              }}
          ...
          <div className="my-6 font-semibold text-lg text-gray-400 ">
            Already have an account?
          </div>
          <div className="border border-gray-400 text-gray-400 font-bold w-full flex items-center justify-center py-4 rounded-full">
